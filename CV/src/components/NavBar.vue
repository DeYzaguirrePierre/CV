<template>
    <header class="fixed top-0 right-0 p-4 z-20 flex items-center">
      ...
      <transition name="slide">
        <nav v-if="isNavOpen" class="fixed top-0 right-0 h-full w-64 green-bg shadow-lg z-10 p-6">
          <ul class="space-y-4">
            <li><RouterLink to="/" class="green-link cursor-pointer">Accueil</RouterLink></li>
            <li><RouterLink to="/competences" class="green-link cursor-pointer">Compétences</RouterLink></li>
  ...
  .hamburger-wrapper {
    position: relative;
    z-index: 30; /* Ensure it is above the nav */
  }
  .hamburger {
    cursor: pointer;
    width: 30px;
